        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg p-6">
            <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $announcement->title) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600 focus:border-teal-500 focus:ring-teal-500">
                    @error('title')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div class="mb-4">
                    <label for="content" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Isi Pengumuman</label>
                    <textarea name="content" id="content" rows="4" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600 focus:border-teal-500 focus:ring-teal-500">{{ old('content', $announcement->content) }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    <select name="level" id="level" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600 focus:border-teal-500 focus:ring-teal-500">
                        ...
                        <option value="warning" {{ old('level', $announcement->level) == 'warning' ? 'selected' : '' }}>Peringatan (Kuning)</option>
                        <option value="danger" {{ old('level', $announcement->level) == 'danger' ? 'selected' : '' }}>Penting (Merah)</option>
                    </select>
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-teal-600 shadow-sm focus:ring-teal-500" {{ old('is_active', $announcement->is_active) ? 'checked' : '' }}>

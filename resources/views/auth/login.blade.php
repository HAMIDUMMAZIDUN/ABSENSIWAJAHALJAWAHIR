<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div>
            ...
            <x-text-input id="email" class="block mt-1 w-full bg-white dark:bg-white text-black dark:text-black" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" />
        ...
        <div class="mt-4">
            ...
            <div class="relative">
                <x-text-input id="password" class="block mt-1 w-full pr-10 bg-white dark:bg-white text-black dark:text-black"
                                type="password"
                                name="password"
        ...
        <div class="flex items-center justify-end mt-4">
            @if (Route::has('password.request'))
                <a class="underline text-sm text-black dark:text-black hover:text-gray-700 dark:hover:text-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('password.request') }}">
                    {{ __('Lupa Kata Sandi?') }}
                </a>
            @endif
